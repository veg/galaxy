...
var _fasta = __webpack_require__(2);

var _fasta2 = _interopRequireDefault(_fasta);
...
var Alignment = function (_Component) {
  ...
  _createClass(Alignment, [{
    key: 'renderAlignment',
    value: function renderAlignment() {
      var alignment_data = this.alignment_data,
          site_size = this.site_size,
          axis_height = this.axis_height;

      if (!alignment_data) return;
      var label_padding = this.props.label_padding;

      var margin = { top: 0, right: 0, bottom: 0, left: 0 };
      this.number_of_sequences = alignment_data.length, this.number_of_sites = alignment_data[0].seq.length, this.height = this.number_of_sequences * site_size, this.alignment_width = this.number_of_sites * site_size, this.names = alignment_data.map(function (d) {
        return d.header;
      });
      var site_scale = d3.scaleLinear().domain([1, this.number_of_sites]).rangeRound([0, this.alignment_width]);

      var label_scale = d3.scaleLinear().domain([1, this.number_of_sequences]).range([0, this.height]);

      var axis_scale = d3.scaleLinear().domain([1, this.number_of_sites]).range([site_size / 2, this.alignment_width - site_size / 2]);

      var labels_svg = d3.select('#labels').attr('height', this.height + margin.top + margin.bottom);

      var labels_g = labels_svg.append('g').attr('transform', 'translate(-' + label_padding + ',' + margin.top + ')');

      var labels = labels_g.selectAll('text').data(this.names).enter().append('text').attr('y', function (d, i) {
        return (i + 1) * site_size;
      }).attr('text-anchor', 'end').attr('dy', -site_size / 3).text(function (d) {
        return d;
      });
      ...
      var computed_width = Math.min(label_width + this.number_of_sites * this.site_size, this.props.width),
      ...
      d3.select('#jsav-div').style('width', computed_width + 'px').style('height', computed_height + 'px');

      var alignment_canvas = d3.select('#alignment').attr('width', computed_width - label_width).attr('height', computed_height - axis_height);
      var context = alignment_canvas.node().getContext("2d");

      labels_svg.attr('width', label_width + 5);
      labels.attr('x', label_width);

      var placeholder_svg = d3.select('#placeholder').attr('width', label_width).attr('height', axis_height);

      var axis_svg = d3.select('#axis').attr('width', this.alignment_width).attr('height', axis_height);

      var axis = d3.axisTop().scale(axis_scale).tickValues(d3.range(1, this.number_of_sites, 2));

      axis_svg.append('g').attr('class', 'axis').attr('transform', 'translate(' + margin.left + ',' + 19 + ')').call(axis);

      d3.select('#placeholder-div').style("width", label_width + "px").style("height", axis_height + "px");

      d3.select('#axis-div').style("width", computed_width - label_width + "px").style("height", axis_height + "px");

      d3.select('#labels-div').style("width", label_width + "px").style("height", computed_height - axis_height + "px");

      d3.select('#alignment-div').style("width", computed_width - label_width + "px").style("height", computed_height - axis_height + "px");
      ...
      if (this.props.centerOnSite) {
        var full_width = this.number_of_sites * this.site_size,
      ...
      if (this.props.centerOnHeader) {
        var header_index = this.names.indexOf(this.props.centerOnHeader) + 1,
    ...
    value: function draw(x, y) {
      var alignment_data = this.alignment_data,
          site_size = this.site_size,
          _props = this.props,
          site_color = _props.site_color,
          text_color = _props.text_color;

      var context = d3.select('#alignment').node().getContext("2d");
      $('#axis-div').scrollLeft(-x);
      $('#labels-div').scrollTop(-y);
      var start_site = Math.max(-Math.floor(x / site_size) - 1, 0),
      ...
      var individual_sites = _.flatten(alignment_data.filter(function (row, i) {
    ...
    value: function componentDidMount() {
      ...
      $('#alignment-div').on('wheel', function (e) {
        e.preventDefault();
        var new_x = self.x + e.originalEvent.deltaX;
        var new_y = self.y + e.originalEvent.deltaY;
        var number_of_sequences = self.alignment_data.length,
    ...
    value: function componentDidUpdate() {
      if (this.props.fasta) {
        ...
        this.alignment_data = (0, _fasta2.default)(this.props.fasta);
        this.axis_height = 20;
        this.site_size = 20;
        this.renderAlignment();
      }
    ...
    value: function render() {
      return _react2.default.createElement(
        ...
        _react2.default.createElement(
          'div',
          { id: 'placeholder-div', className: 'jav-container' },
          _react2.default.createElement('svg', { id: 'placeholder' })
        ),
        _react2.default.createElement(
          'div',
          { id: 'axis-div', className: 'jav-container', style: { overflow: "scroll", overflowX: "hidden" } },
          _react2.default.createElement('svg', { id: 'axis' })
        ),
        _react2.default.createElement(
          'div',
          { id: 'labels-div', className: 'jav-container', style: { overflow: "scroll", overflowY: "hidden" } },
          _react2.default.createElement('svg', { id: 'labels' })
        ),
        _react2.default.createElement(
          'div',
          { id: 'alignment-div', className: 'jav-container', style: { overflow: "scroll" } },
          _react2.default.createElement('canvas', { id: 'alignment' })
        )
      );
    }
  }]);
...
var nucleotide_colors = {
  ...
  return nucleotide_text_colors[character];
    ...
    highlight_codon_color = function highlight_codon_color(character, position, header) {
  if (header == 'DUCK_VIETNAM_272_2005' && Math.floor((position - 1) / 3) == 3) return 'red';
  return character == "-" ? "white" : "GhostWhite";
},
    highlight_codon_text_color = function highlight_codon_text_color(character, position, header) {
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../node_modules/css-loader/index.js!./jav.css", function() {
			var newContent = require("!!../node_modules/css-loader/index.js!./jav.css");
