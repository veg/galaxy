define("reports/run_stats", ["exports", "../libs/d3"], function(exports, _d) {
    "use strict";
    ...
    exports.create_histogram = create_histogram;
    ...
    function create_chart(inp_data, name, time, title) {
        ...
        function click() {
            ...
            d3.selectAll("." + classes[0]).filter("." + classes[1]).style("cursor", "zoom-in").transition().duration(750).attr("height", chart_height).attr("width", chart_width);

            d3.select(this).style("cursor", "default").transition().duration(750).attr("height", chart_height * chart_zoom).attr("width", chart_width * chart_zoom);
        }
        ...
        var chart = d3.select("#" + name).attr("width", chart_width).attr("height", chart_height).attr("preserveAspectRatio", "xMidYMin").attr("viewBox", "0 0 " + chart_width + " " + chart_height).on("click", click);
        ...
        var bar = chart.selectAll("g").data(data).enter().append("g").attr("transform", function(d, i) {
            ...
            var wdth = i * 4 + 10;
            d3.select(_d.event.target.parentElement).select(".tool_tip").select("text").attr("transform", "translate( " + (margin.left - 5) + ", " + (height - d * zoom + margin.top + 10) + " )").attr("visibility", "visible").text(d);

            d3.select(_d.event.target.parentElement).select(".tool_tip").attr("width", wdth + "px").attr("height", "15px").select("rect").attr("transform", "translate( " + (margin.left - wdth) + ", " + (height - d * zoom + margin.top) + " )").attr("width", wdth + "px").attr("height", "15px").attr("fill", "#ebd9b2");
        }).on("mouseleave", function(d) {
            // Remove tool tip
            d3.select(_d.event.target.parentElement).select(".tool_tip").select("text").attr("visibility", "hidden");

            d3.select(_d.event.target.parentElement).select(".tool_tip").select("rect").attr("width", "0").attr("height", "0").attr("fill", "").text(d);
        });

        // Add a title to the chart
        chart.append("g").append("text").attr("class", "title").attr("text-anchor", "end").attr("transform", function() {
            return "translate( " + width + ",15 )";
        }).text(title);

        // Add an x axis line to the chart
        chart.append("g").attr("class", "axis").append("path").attr("class", "x").attr("d", function() {
            var m_x = margin.left;
        ...
        chart.append("g").attr("class", "y axis").attr("id", "y_" + name).attr("text-anchor", "end").attr("transform", "translate( " + margin.left + "," + margin.top + ")").call(yAxis).select(".domain");

        // Put a title for y axis on chart
        chart.append("g").append("text").attr("class", "ax_title").attr("transform", function() {
        ...
        if (time == "hours") {
            // Append hour lines
            bar.append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", 3).attr("stroke", "black").attr("stroke-width", 1).attr("pointer-events", "none").attr("transform", function() {
                return "translate( " + barWidth / 2 + ", " + height + ")";
            });

            // Append hour numbers
            bar.append("text").attr("fill", "rgb(0,0,0)").attr("transform", "translate( 10, " + (height + 10) + " )").text(function(d, i) {
                var time = "0000";
            ...
            var curr_day = "";
            first = false;
            bar.append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", function(d, i) {
                var _y2 = 0;
                ...
                return _y2;
            }).attr("stroke", "black").attr("stroke-width", 1).attr("pointer-events", "none").attr("transform", function() {
            ...
            var curr_day_text = "";
            first = false;
            bar.append("text").attr("fill", "rgb(0,0,0)").attr("pointer-events", "none").text(function(d, i) {
                var time = "";
                var locale = "en-us";

                if (hours_array[i].getDate() != curr_day_text) {
                    time = String(hours_array[i].toLocaleString(locale, {
                        month: "long"
                    }));
            ...
            // Append day numbers
            bar.append("text").attr("fill", "rgb(0,0,0)").attr("transform", "translate( 9, " + (height + 10) + " )").text(function(d, i) {
                var time = "0000";
            ...
            var curr_month_text = "";
            first = false;
            bar.append("text").attr("fill", "rgb(0,100,0)").attr("pointer-events", "none").text(function(d, i) {
                var time = "";
                var locale = "en-us";

                if (days_array[i].getMonth() != curr_month_text) {
                    time = String(days_array[i].toLocaleString(locale, {
                        month: "long"
                    }));
        ...
        if (name == "jc_dy_chart" || name == "jc_hr_chart") {
            d3.select("#" + name).attr("height", chart_height * chart_zoom).attr("width", chart_width * chart_zoom).style("cursor", "default");
        }
    }
    ...
    function create_histogram(inp_data, name, title) {
        ...
        var chart = d3.select("#" + name).attr("viewBox", "0 0 " + chart_width + " " + chart_height).attr("width", chart_width).attr("height", chart_height).attr("preserveAspectRatio", "xMidYMin").on("click", click);

        // Put title on chart
        chart.append("g").append("text").attr("class", "title").attr("transform", function() {
            return "translate( " + width + ",15 )";
        }).text(title);

        // Put bars on chart
        var bar = chart.selectAll(".bar").data(data).enter().append("g").attr("class", "bar").attr("transform", function(d) {
            ...
            var wdth = i * 4 + 10;
            d3.select(_d.event.target.parentElement).select(".tool_tip").select("text").attr("transform", "translate( " + (margin.left - 5) + ", " + (height - d.length * zoom + margin.top + 10) + " )").attr("visibility", "visible").text(d.length);

            d3.select(_d.event.target.parentElement).select(".tool_tip").attr("width", wdth + "px").attr("height", "15px").select("rect").attr("transform", "translate( " + (margin.left - wdth) + ", " + (height - d.length * zoom + margin.top) + " )").attr("width", wdth + "px").attr("height", "15px").attr("fill", "#ebd9b2");
        }).on("mouseleave", function() {
            // Remove tool tip
            d3.select(_d.event.target.parentElement).select(".tool_tip").select("text").attr("visibility", "hidden");

            d3.select(_d.event.target.parentElement).select(".tool_tip").select("rect").attr("width", "0").attr("height", "0").attr("fill", "");
        });
        ...
        // Add color to bar
        bar.append("rect").attr("x", 1).attr("width", bar_x - 1).attr("height", function(d) {
            return height - y(d.y);
        });
        ...
        // Add x axis to chart
        chart.append("g").attr("class", "x axis").attr("id", "x_" + name).attr("transform", "translate( " + margin.left + "," + (+height + margin.top) + ")").call(xAxis);
        ...
        // Add y axis to chart
        chart.append("g").attr("class", "y axis").attr("id", "y_" + name).attr("transform", "translate( " + margin.left + "," + margin.top + ")").call(yAxis);
...
//# sourceMappingURL=../../maps/reports/run_stats.js.map
