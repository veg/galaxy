define("galaxy.pages", ["exports", "utils/localization", "mvc/grid/grid-view", "libs/jquery/jquery.form", "libs/jquery/jstorage", "libs/jquery/jquery.wymeditor", "libs/jquery/jquery.autocomplete"], function(exports, _localization, _gridView) {
    "use strict";

    Object.defineProperty(exports, "__esModule", {
        value: true
    });
    exports.default = pagesEditorOnload;
    ...
    WYMeditor.editor.prototype.dialog = function(dialogType, dialogFeatures, bodyHtml) {
        ...
        if (dialogType == WYMeditor.DIALOG_LINK) {
            ...
            show_modal("Create Link", "<div><div><label id='link_attribute_label'>URL <span style='float: right; font-size: 90%'><a href='#' id='set_link_id'>Create in-page anchor</a></span></label><br><input type='text' class='wym_href' value='" + curURL + "' size='40' /></div><div><label>Title</label><br><input type='text' class='wym_title' value='" + curTitle + "' size='40' /></div><div>", {
                "Make link": function MakeLink() {
                    // Get URL, name/title.
        ...
        if (dialogType == WYMeditor.DIALOG_IMAGE) {
            if (wym._selected_image) {
                $(wym._options.dialogImageSelector + " " + wym._options.srcSelector).val($(wym._selected_image).attr(WYMeditor.SRC));
                $(wym._options.dialogImageSelector + " " + wym._options.titleSelector).val($(wym._selected_image).attr(WYMeditor.TITLE));
                $(wym._options.dialogImageSelector + " " + wym._options.altSelector).val($(wym._selected_image).attr(WYMeditor.ALT));
            }
            show_modal("Image", "<div class='row'>" + "<label>URL</label><br>" + "<input type='text' class='wym_src' value='' size='40' />" + "</div>" + "<div class='row'>" + "<label>Alt text</label><br>" + "<input type='text' class='wym_alt' value='' size='40' />" + "</div>" + "<div class='row'>" + "<label>Title</label><br>" + "<input type='text' class='wym_title' value='' size='40' />" + "</div>", {
                Insert: function Insert() {
                    var sUrl = $(wym._options.srcSelector).val();
                    if (sUrl.length > 0) {
                        wym._exec(WYMeditor.INSERT_IMAGE, sStamp);
                        $("img[src$=" + sStamp + "]", wym._doc.body).attr(WYMeditor.SRC, sUrl).attr(WYMeditor.TITLE, $(wym._options.titleSelector).val()).attr(WYMeditor.ALT, $(wym._options.altSelector).val());
                    }
                    hide_modal();
                },
                Cancel: function Cancel() {
        ...
        if (dialogType == CONTROLS.DIALOG_HISTORY_LINK || dialogType == CONTROLS.DIALOG_DATASET_LINK || dialogType == CONTROLS.DIALOG_WORKFLOW_LINK || dialogType == CONTROLS.DIALOG_PAGE_LINK || dialogType == CONTROLS.DIALOG_VISUALIZATION_LINK) {
            ...
            var grid = new _gridView2.default({
                url_base: item_info.list_ajax_url,
                embedded: true
            });
            Galaxy.modal.show({
                title: "Insert Link to " + item_info.singular,
                body: $("<div/>").append(grid.$el).append($("<div/>").append('<input id="make-importable" type="checkbox" checked/>').append("Make the selected " + item_info.plural.toLowerCase() + " accessible so that they can viewed by everyone.")),
                closing_events: true,
                buttons: {
                    Insert: function Insert() {
                ...
                buttons: {
                    Embed: function Embed() {
                        ...
                        grid.$("input[name=id]:checked").each(function() {
                            ...
                            var item_embed_html = ["<div id='", item_elt_id, "' class='embedded-item ", item_info.singular.toLowerCase(), " placeholder'>", "<p class='title'>", "Embedded Galaxy ", item_info.singular, " '", item_name, "'", "</p>", "<p class='content'>", "[Do not edit this block; Galaxy will fill it in with the annotated ", item_info.singular.toLowerCase(), " when it is displayed.]", "</p>", "</div>"].join("");
    ...
    function pagesEditorOnload() {
        // Generic error handling
        $(document).ajaxError(function(e, x) {
            // console.log( e, x );
            var message = x.responseText || x.statusText || "Could not connect to server";
            show_modal("Server error", message, {
                "Ignore error": hide_modal
            });
            return false;
        });
        // Create editor
        $("[name=page_content]").wymeditor({
            ...
            // + WYMeditor.HTML
            boxHtml: "<table class='wym_box' width='100%' height='100%'><tr><td><div class='wym_area_top'>" + WYMeditor.TOOLS + "</div></td></tr><tr height='100%'><td><div class='wym_area_main' style='height: 100%;'>" +
                // + WYMeditor.HTML
            ...
            toolsItems: [{
                ...
                name: "Superscript",
                title: (0, _localization2.default)("Superscript"),
                css: "wym_tools_superscript"
            }, {
                name: "Subscript",
                title: (0, _localization2.default)("Subscript"),
        ...
        $("#close-button").click(function() {
            ...
            if (changed) {
                ...
                show_modal("Close editor", "There are unsaved changes to your page which will be lost.", {
                    Cancel: hide_modal,
        ...
        make_popupmenu(insert_link_menu_button, {
            "Insert History Link": function InsertHistoryLink() {
                editor.dialog(CONTROLS.DIALOG_HISTORY_LINK);
            },
            "Insert Dataset Link": function InsertDatasetLink() {
                editor.dialog(CONTROLS.DIALOG_DATASET_LINK);
            },
            "Insert Workflow Link": function InsertWorkflowLink() {
                editor.dialog(CONTROLS.DIALOG_WORKFLOW_LINK);
            },
            "Insert Page Link": function InsertPageLink() {
                editor.dialog(CONTROLS.DIALOG_PAGE_LINK);
            },
            "Insert Visualization Link": function InsertVisualizationLink() {
        ...
        make_popupmenu(embed_object_button, {
            "Embed History": function EmbedHistory() {
                editor.dialog(CONTROLS.DIALOG_EMBED_HISTORY);
            },
            "Embed Dataset": function EmbedDataset() {
                editor.dialog(CONTROLS.DIALOG_EMBED_DATASET);
            },
            "Embed Workflow": function EmbedWorkflow() {
                editor.dialog(CONTROLS.DIALOG_EMBED_WORKFLOW);
            },
            "Embed Visualization": function EmbedVisualization() {
