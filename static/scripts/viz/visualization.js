define("viz/visualization", ["exports", "utils/localization", "libs/underscore", "mvc/dataset/data", "viz/trackster/util", "utils/config", "mvc/grid/grid-view", "mvc/ui/ui-tabs"], function(exports, _localization, _underscore, _data, _util, _config, _gridView, _uiTabs) {
    "use strict";
    ...
    var select_datasets = function select_datasets(filters, success_fn) {
        ...
        tabs.add({
            id: "libraries",
            title: (0, _localization2.default)("Libraries"),
            $el: $("<div/>").append(library_grid.$el)
        });

        // modal
        Galaxy.modal.show({
            title: (0, _localization2.default)("Select datasets for new tracks"),
            body: tabs.$el,
            closing_events: true,
            buttons: {
    ...
    var GenomeReferenceDataManager = GenomeDataManager.extend({
        ...
        load_data: function load_data(region, mode, resolution, extra_params) {
            ...
            return region.length() <= 100000 ? GenomeDataManager.prototype.load_data.call(this, region, mode, resolution, extra_params) : {
                data: null,
                region: region
            };
        }
    ...
    var TrackBrowserRouter = Backbone.Router.extend({
        initialize: function initialize(options) {
            ...
            this.view.on("navigate", function(new_loc) {
                _this.navigate(new_loc);
            });
        },
